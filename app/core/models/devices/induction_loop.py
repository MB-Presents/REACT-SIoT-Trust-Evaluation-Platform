...
from scenarios.canberra_case_study.core.devices import INDUCTION_SENSOR_SPECIFICATION
import utils.logging as logger

class InductionLoop(GenericDevice):

    def __init__(self,
                 induction_loop_id: str,
                 observedStreet : str,
        ...
        super().__init__(
            device_id=induction_loop_id, 
            device_type=DeviceType.INDUCTION_LOOP, 
            color=color, 
            profile_config=profile_config,
            simulation_context=simulation_context)

        self.with_position(position=position)
        assert self._position is not None, "Position must be set"

        self._observed_street : str = observedStreet
        self._captured_vehicles_features : Dict[str,Any] = {}

        traci.poi.add(self._id, self._position[0], self._position[1], self._color, poiType=self._type.name, width=50000, height=50000)
...
#                                     #   date_of_manufacture=induction_conf['date_of_manufacture'], 
#                                     #   last_maintenance_date=induction_conf['last_maintenance_date'],
#                                     #   next_maintenance_date=induction_conf['next_maintenance_date']
#                                       )
...
#             print(f'Device created: {induction_id} with device mapping id: {self._induction_loops[induction_id]._device_map_id}')  
    ...
    #         induction_loop._services[Service.Inductive_Vehicle_Detection_Service]._function[Function.GET_INDUCTIVE_OBJECT_COUNT](induction_loop, self._devices)
