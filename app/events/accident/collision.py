...
from data_models.events.simulation_events import EventType, SimulationEvent, SimulationEventManager
...
def is_valid_simulation_context(vehicle_key: str, events_manager: SimulationEventManager):
    return not events_manager.exists_for_vehicle(vehicle_key)
...
def is_fake_collision_condition_fulfilled(veh_id: str, vehicles: Dict[str, Vehicle], events_manager: SimulationEventManager):
    ...
    vehicle_is_not_collided = not events_manager.exists_for_vehicle(veh_id)
...
def is_valid_time_constraints(vehicles : Dict[str, Vehicle]) -> bool:
    ...
    is_time_interval_triggered = sc.TIME % Settings.INTERVAL_OF_ACCIDENTS == 0
...
def is_vehicles_stoppable_at_edge(collision_vehicle: Vehicle, collision_vehicle_key: str, route: List[str], route_index: int, accumulated_length: float) -> Tuple[bool, Optional[str], Optional[float], float]:
    ...
    try:
        ...
        if length_to_end_of_current_edge > required_distance:
            ...
            # print(f"{sc.TIME_STRING} \t| VALID COLLISION | Collision vehicle: {collision_vehicle_key}, Edge: {edge_id} at Position : {stop_position}, Accumulated length {accumulated_length}")
...
def find_false_reporter(simulation_event_manager : SimulationEventManager ):
    ...
    legit_accident_events : Dict[str,SimulationEvent] = simulation_event_manager.get_simulation_events_by_event_and_authenticity(EventType.COLLISION)
    ...
    while len(reporter_candidates) > 0:
        ...
        surrounding_vehicles = network_manager.get_surrounding_vehicles(reporter_id, reporter, devices)
        ...
        reporter = SendersEntity(reporter, AuthenticityRole.UNAUTHENTIC,sc.TIME)
...
def vehicle_conditions_met(veh_id :str, vehicle : Vehicle, vehicle_object : dict, simulation_event_manager : SimulationEventManager):
    ...
    is_legit_vehicle = collision_utils.is_on_lane(vehicle._edge_id)
    
    is_valid_lane_position = collision_utils.valid_lane_position(vehicle)
    is_allowed_on_lane = collision_utils.vehicle_allowed_on_lane(vehicle)
    is_routable_for_emergency_vehicle = collision_utils.is_accident_routable_for_emergency_vehicle(vehicle, edgeId=vehicle._edge_id, lane_position=20)
