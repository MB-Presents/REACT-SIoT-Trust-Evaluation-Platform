...
from dataset_builder.utils.path_helper import get_dataset_directory_path
# %%
...
def generate_node_classification_dataset(devices_df : pd.DataFrame):
    
    print(devices_df.columns)
    
    
    # TODO: Adjust node features
    
    col_to_keep = ['device_id', 'time', 'situation_label', 'color', 'device_id_initial']
    other_feature_columns = devices_df.columns.difference(col_to_keep).tolist()
    
    
    node_features = devices_df[['device_id', 'time', 'situation_label'] + other_feature_columns]
    return node_features

def generate_edge_prediction_dataset(relationships_df : DataFrame, negative_samples_df : DataFrame):
    
    print(devices_df.columns)
    
    
    relationships_df['edge_type'] = 'positive'
    negative_samples_df['edge_type'] = 'negative'
    edge_prediction_df = pd.concat([relationships_df, negative_samples_df])
...
def generate_subgraph_classification_dataset(devices_df, relationships_df):
    ...
    for ts in unique_timestamps:
        G = Graph()
        devices_at_ts = devices_df[devices_df['time'] == ts]
        relationships_at_ts = relationships_df[relationships_df['time'] == ts]
        
        for _, row in relationships_at_ts.iterrows():
            if row['distance'] <= 50:
                G.add_edge(row['origin_device_id'], row['target_device_id'])
        
        for component in connected_components(G):
            subgraph_nodes = list(component)
            subgraph_features = devices_at_ts[devices_at_ts['device_id'].isin(subgraph_nodes)]
            subgraphs.append({'time': ts, 'nodes': subgraph_nodes, 'features': subgraph_features})
...
DATA_DIR = get_dataset_directory_path(BUILD_DEVICE)
os.path.join(DATA_DIR, 'devices.csv')




DATA_DIR_OUTPUT = os.path.join(DATA_DIR, data_name + '_' +  DATASET_OPTION)
...
if DATASET_OPTION == 'node_classification':
    node_classification_dataset = generate_node_classification_dataset(devices_df)
    

# elif DATASET_OPTION == 'edge_prediction':
#     edge_prediction_dataset = generate_edge_prediction_dataset(relationships_df, negative_samples_df)
    # Save or further process edge_prediction_dataset

# elif DATASET_OPTION == 'subgraph_classification':
#     subgraph_classification_dataset = generate_subgraph_classification_dataset(devices_df, relationships_df)
    # Save or further process subgraph_classification_dataset
