...
def log_status(accident: SendingPacket):
    message = {
        'accident_id': str(accident.report_id),
    ...
    logger.info(ObjectType.DEBUG_ACCIDENT_STATUS, message)

def set_vehicle_stop(self, emergency_veh_id, selected_edge: str, lane_position: float, laneIndex: int, parking_duration: int):
    vehicle_method = vehicle.resume if vehicle.isStopped(emergency_veh_id) else vehicle.replaceStop
    vehicle_method(emergency_veh_id, nextStopIndex=0, edgeID=selected_edge,
